...
spawn(function()
	local Loop, Range, SortMode, TeamCheck, Block, TargetESP = nil, nil, nil, nil, nil, nil
	local Sword = nil
	local TargetHighlight = Instance.new("Highlight")
	TargetHighlight.FillColor = Color3.fromRGB(255, 0, 127)
				...
				Loop = RunService.RenderStepped:Connect(function()
					...
					if Target then
						...
						if Sword then
							...
							if TargetESP and TargetHighlight then
	...
	local KillAuraESP = KillAura:CreateMiniToggle({
		Name = "ESP",
		Callback = function(callback)
			if callback then
				TargetESP = callback
				...
				Loop = RunService.Heartbeat:Connect(function()
					...
					Humanoid:GetPropertyChangedSignal("FloorMaterial"):Connect(function()
						if Humanoid.FloorMaterial ~= Enum.Material.Air then
							...
							PositionHighlight.Position = LastPosition
						end
					end)
					if HumanoidRootPart.Position.Y < -136 then
						...
						elseif Mode == "Tween" then
							TweenService:Create(HumanoidRootPart, TweenInfo.new(0.2), {CFrame = CFrame.new(LastPosition)}):Play()
						end
					end
				end)
			else
